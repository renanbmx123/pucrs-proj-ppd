...
void main(int argc, char** argv){
  ...
  if ( my_rank != 0 ){
        MPI_Recv(vetor, VETOR_SIZE, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);  // nao sou a raiz, tenho pai
  ...
  if( tam_vetor <= delta ){
        ...
        float tam_filhos;
        if(LOCAL){      //Se for local, ordena da posicao 0 ate o delta, e divide o resto igualmente entre os filhos
                tam_local = delta;
                tam_divide = tam_vetor-delta;
        }else{          //Se nao for local, divide todo o vetor igualmente entre os filhos
                tam_local = 0;
        ...
        else{							//se nao for um tamanho de vetor com casa decimal
        ...
        if(LOCAL){				//se for execucao local, ordeno a minha parte do vetor
